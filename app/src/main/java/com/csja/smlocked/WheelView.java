...
//                }
    ...
    private void initData() {
        ...
        if (currentId == -1) {
            ...
            if (currentId >= dataList.size()) {
                throw new IllegalArgumentException("parm");
            }
            int height = itemNumber / 2 * unitHeight;
            for (int i = currentId; i > -1; i--) {
                ItemObject itmItemObject = new ItemObject();
                itmItemObject.id = i;
                itmItemObject.itemText = dataList.get(i);
                itmItemObject.x = 0;
                itmItemObject.y = (height) - (currentId - i) * unitHeight;
                itemList.add(itmItemObject);
            }

            for (int i = currentId + 1; i < dataList.size(); i++) {
                ItemObject itmItemObject = new ItemObject();
                itmItemObject.id = i;
                itmItemObject.itemText = dataList.get(i);
                itmItemObject.x = 0;
                itmItemObject.y = (height) + (i - currentId) * unitHeight;
                itemList.add(itmItemObject);
            }
        }
     ...
     * @param data
     */
    public void setData(List<String> data, int currentId) {
        if (data != null) {
            this.dataList.clear();
     ...
     * @param data
     */
    public void resetData(List<String> data, int selectId) {
